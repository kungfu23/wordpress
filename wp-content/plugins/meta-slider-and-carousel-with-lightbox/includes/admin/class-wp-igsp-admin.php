...
class Wp_Igsp_Admin {
	...
	function __construct() {
		...
		add_action ( 'admin_init', array($this,'wp_igsp_register_settings') );
	 ...
	 * @since 1.0.0
	 */
	function wp_igsp_register_settings() {
		...
		if( !empty($attachment_id) && !empty($form_data_arr) ) {
			...
			if( isset($wp_igsp_attachment_post->post_type) && $wp_igsp_attachment_post->post_type == 'attachment' ) {
				...
				if( !is_wp_error( $update ) ) {
					...
					update_post_meta( $attachment_id, $prefix.'attachment_link', wp_igsp_slashes_deep($form_data_arr['wp_igsp_attachment_link']) );
